...
p = re.compile(r'ORIGIN(.*\D)$')                                                         # regex to extract seqeunce data 
sequence = []
sequence_1 = []
for i in seq:
        for match in p.finditer(i):
                seq_1 = re.sub("[^atgcn-]", "", match.group(1))         # regex to remove digits from sequence
...
print('complete sequence table import')         # confirm data has finished importing 
